...
const Card: React.FC<CardProps> = ({ title, value, percentage, icon:Icon }) => {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-[20px] p-6 shadow-sm flex flex-col justify-between">
      <div className="flex items-center mb-1">
        <div className="text-gray-500 text-lg font-large"><Icon className='w-3 h-3' /></div>
        <h3 className="text-gray-500 text-xs/[30px] font-medium uppercase ml-1">{title}</h3>
      </div>
      {/* Title and Value */}
      <div>
        <p className="text-3xl/[38px] font-medium my-1">{value}</p>
        <div
